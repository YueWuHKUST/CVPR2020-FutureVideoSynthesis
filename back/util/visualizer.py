...
class Visualizer():
    ...
    def print_current_errors_mean_finetune(self, epoch, i, errors, t, all_loss_0, all_loss_1):
        message = '(epoch: %d, iters: %d, time: %.3f) ' % (epoch, i, t)
        cnt = 0
        for k, v in sorted(errors.items()):
            loss_k = all_loss_0[:, cnt]
            cnt = cnt + 1
            mean_loss = np.mean(loss_k[np.where(loss_k)])
            message += '%s: %.3f ' % (k, mean_loss)
        print(message)
    ...
    def save_test_images(self, save_dir, visuals, index):
        dir = save_dir# + "/%04d/"%index
        util.mkdir(dir)
        for label, image_numpy in visuals.items():
            save_ext = 'png'
