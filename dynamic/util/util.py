...
def tensor2occ(image_tensor, imtype=np.uint8):
    if isinstance(image_tensor, list):
        image_numpy = []
        for i in range(len(image_tensor)):
            image_numpy.append(tensor2im(image_tensor[i], imtype, normalize))
    ...
    if len(image_tensor.size()) == 4:
        image_tensor = image_tensor[0]
    image_numpy = image_tensor.cpu().float().numpy()
    image_numpy = np.mean(image_numpy, axis=0)
    image_numpy = (image_numpy > 0.5).astype(np.int32)
...
def numpy2mask(image_numpy, imtype=np.uint8, normalize=True):
    image_numpy = np.transpose(image_numpy, (1, 2, 0)) * 255.0
    #image_numpy = (np.transpose(image_numpy, (1, 2, 0)) * std + mean)  * 255.0        
    image_numpy = np.clip(image_numpy, 0, 255)
    if image_numpy.shape[2] == 1:        
...
def tensor2graylabel(output, imtype=np.uint8):
    if isinstance(output, torch.autograd.Variable):
        output = output.data
    if len(output.size()) == 4:
        output = output[0]
