...
class TestTemporalDataset(BaseDataset):
    ...
    def initialize(self, opt, flownet):
        ...
        if opt.seq == 0:
            self.all_image_paths = self.load_all_image_paths('./test_cityscapes_my_back_000_100.npy')
        elif opt.seq == 100:
            self.all_image_paths = self.load_all_image_paths('./test_cityscapes_my_back_100_200.npy')
        elif opt.seq == 200:
            self.all_image_paths = self.load_all_image_paths('./test_cityscapes_my_back_200_300.npy')
        elif opt.seq == 300:
            self.all_image_paths = self.load_all_image_paths('./test_cityscapes_my_back_300_400.npy')
        elif opt.seq == 400:
            self.all_image_paths = self.load_all_image_paths('./test_cityscapes_my_back_400_500.npy')


        self.my_back_root = "/root/data/result/cityscapes/test_32/"
    ...
    def __getitem__(self, index):
        ...
        Backs = torch.cat([self.get_image(back_paths[p], t_bic) for p in range(tIn)], dim=0)
        # Load target back here
        cnt_back_dir = self.my_back_root + "%04d/"%(index+self.seq)
        target_back = torch.cat([self.get_image(cnt_back_dir + "warp_image_bwd_inpainted_all_fore_%02d.png"%p, t_bic) for p in range(tOut)], dim=0)
        target_all_back = torch.cat([self.get_image(cnt_back_dir + "warp_image_bwd_inpainted_%02d.png"%p, t_bic) for p in range(tOut)], dim=0)
        
        Backs = torch.cat([Backs, target_back], dim=0)
        ...
        return_list = {'Image': Images,'Back': Backs, 'All_Back': target_all_back, 'Mask': Masks, 'Semantic': Semantics, 'Combine': Combines, 'LastObject': LastObjects, 'Depths': Depths, 'Classes': Classes, 'LastMasks': LastMasks}
        return return_list
    ...
    def check_valid(self, category, frames, dynamic_list):
        ...
        if last_mask.sum() < self.mask_threshold:
            return False
        must_moving_class = [11,12]# classes of person, rider and bike and moto, consistent with background prediction
        if category in must_moving_class:
            return True
        #if category not in [13,14,15,16]:
        #    return False
        #flag = self.whether_move(frames)
        #if flag is False:
        #    return False 
