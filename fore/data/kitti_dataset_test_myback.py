...
#Test dataloader for testing the foreground prediction
#Each sequence return a batch of data
#Can choose two type: pre-inpainted image, or the inpainted result from background predicton model
#Mode 1 used to determine whether foreground prediction work
#Mode 2 used to determine whether overall model work
#Each batch of data will run multiple of times and ordered by the depth to generate the combined image.
...
class TestKittiDataset(BaseDataset):
    ...
    # Test dict structure
    # ((images, semantics, backs, depths, dynamics, processed_list, videoi, framej))
    def initialize(self, opt, flownet):
        ...
        self.tIn = opt.tIn
        self.tOut = opt.tOut
        self.mask_threshold = int(0.005 * self.height*self.width)
    ...
    def __getitem__(self, index):
        ...
        return_list = {'Back': Backs, 'Mask': Masks, 'Semantic': Semantics, 'Combine': Combines, 'LastObject': LastObjects, 'Depths': Depths, 'Classes': Classes, 'LastMasks': LastMasks}
        return return_list
