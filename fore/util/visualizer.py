...
class Visualizer():
    ...
    def print_current_errors(self, epoch, i, errors, t, all_loss):
    ...
    def save_test_images(self, save_dir, visuals, index, pred_id):
        dir = save_dir + "/%04d/"%index
        util.mkdir(dir)
        for label, image_numpy in visuals.items():
            save_ext = 'png'
            image_name = '%s_%03d.%s' % (label, pred_id, save_ext)
